...
// Filter data for NTC sensor digital signal processing
static FilterData_t g_ntc_filter;
...
void sensor_module_init(const sensor_config_t* config) 
{
    ...
    g_sensor_data.filtered_raw = 0;
    g_sensor_data.physical_value = 0.0f;
    g_sensor_data.filtered_value = 0.0f;
...
void sensor_get_report(char* buffer, size_t buffer_size) 
{
    ...
    float raw_temperature = g_sensor_data.physical_value;    // Raw temperature
    float filtered_temperature = g_sensor_data.filtered_value; // Filtered temperature
    float percentage = sensor_temperature_to_percentage(filtered_temperature);
    
    // Convert to integers for Arduino printf compatibility
    int temp_raw_int = (int)(raw_temperature * 10.0f);
...
// Initialize digital filters for sensor signal processing
void sensor_init_filters(void) {
    filter_init(&g_ntc_filter);
}

// Enhanced sensor reading with digital signal processing
...
#if ENABLE_SALT_PEPPER_FILTER
        // Apply salt and pepper filter (median filter) to raw ADC data
        g_sensor_data.filtered_raw = salt_pepper_filter(&g_ntc_filter, raw);
#else
        g_sensor_data.filtered_raw = raw;
#endif
        
        // Convert filtered raw to voltage
        float voltage = sensor_raw_to_voltage(g_sensor_data.filtered_raw);
        
        // Convert voltage to temperature (Celsius) for NTC temperature sensor
        float temperature = sensor_voltage_to_temperature(voltage);
        g_sensor_data.physical_value = temperature;
        
#if ENABLE_MOVING_AVERAGE
        // Apply weighted moving average filter to temperature
        g_sensor_data.filtered_value = weighted_moving_average(&g_ntc_filter, temperature);
#else
        g_sensor_data.filtered_value = temperature;
#endif

#if ENABLE_SATURATION
        // Apply saturation to filtered temperature
        g_sensor_data.filtered_value = apply_saturation(g_sensor_data.filtered_value,
                                                      SENSOR_MIN_TEMP_SATURATION,
                                                      SENSOR_MAX_TEMP_SATURATION);
#endif
        
        // Check if filtered temperature is valid
        if (sensor_is_temperature_valid(g_sensor_data.filtered_value)) {
            g_sensor_data.status = SENSOR_STATUS_OK;
            g_sensor_data.error_count = 0; // Reset error count on successful read
        } else {
            g_sensor_data.status = SENSOR_STATUS_OUT_OF_RANGE;
